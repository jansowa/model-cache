...
class HashMapCacheModelSpec extends Specification{
    ...
    void "Should load cache from device"()
    {
        ...
        then:
            ...
            loadedCacheModel
                ...
                .filePath)
                .get()
                .getName() == sampleData[0].getName()
        loadedCacheModel
                .read(sampleData[1]
                .filePath)
                .get()
                .getCreationTime() == sampleData[1].getCreationTime()
    ...
    void "Should find oldest file"()
    {
        given:
            cacheModel.put(sampleData[1].withLastUsageTime(new Date()))
            Thread.sleep(100)
            cacheModel.put(sampleData[0].withLastUsageTime(new Date()))
            Thread.sleep(100)
            cacheModel.put(sampleData[2].withLastUsageTime(new Date()))
        when:
            String oldestFilePath = cacheModel.findOldestFile()

        then:
            oldestFilePath == sampleData[1].filePath
    }
}